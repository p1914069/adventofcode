...
tableaux_gagnants = set()
tableau_gagnant = 0
num_marque = 0

n = 0

while len(tableaux_gagnants) < len(tableaux):
    ...
    for i in range(len(tableaux)):
        for j in range(5):
            if (tableaux[i][j][0] == -1 and
                ...
                tableaux[i][j][4] == -1) and \
                    (i not in tableaux_gagnants):

                tableaux_gagnants.add(i)
                if len(tableaux_gagnants) == len(tableaux):
                    tableau_gagnant = i
